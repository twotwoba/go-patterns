...
/* ============== 理论 ============== */
// 状态模式通过将**与特定状态相关的行为**封装到独立的**状态对象**中来解决大量if/else问题。
// 主对象（我们称之为**上下文 Context**）不再包含任何 `if/else` 逻辑，而是简单地将所有请求**委托**给它当前持有的状态对象来处理。
/*
	入口类包含子类的接口引用
...
type Context struct {
	State       ActionState // 维护state接口实例
	HealthValue int
}

// 上下文总是把行为委托给 State 对象
func (a *Context) View() {
	a.State.View()
}
